...
export function AuthenticatedLayout() {
  ...
  if (me.isLoading) return <main className="center-page"><LoadingState label="Starting your session" /></main>
  if (me.error instanceof ApiError && me.error.status === 401) {
    void navigate({ to: '/login', replace: true })
    return <main className="center-page"><LoadingState label="Opening sign in" /></main>
  }
  if (me.isError) {
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <Link className="brand" to="/guilds" aria-label="Channel Bot Console home">
